...
public class GlobalBlogExceptionController  {
    ...
    * exception handler. This exception is typically thrown when validation errors occur during the
    * processing of a request in a Spring MVC controller method.
    * @return A list of CustomException objects containing error code, error message, and field
    * information for each validation error encountered. This list is returned in a ResponseEntity with
    ...
    */
    ...
    public ResponseEntity<?> handleValidationErrors(HandlerMethodValidationException ex) {
        ...
        List<?> errList = ex.getAllErrors().stream()
                .map(error -> CustomException.builder()
                        .errorCode(errorCode)
    ...
    public ResponseEntity<?> handleValidationErrors(HttpMessageNotReadableException ex,WebRequest request) {
        ...
        return new ResponseEntity<>(List.of(customException), new HttpHeaders(), HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<Map<String, Object>> handleValidationExceptions(MethodArgumentNotValidException ex) {
        ...
        response.put("errors", errors.stream().map(error -> {
